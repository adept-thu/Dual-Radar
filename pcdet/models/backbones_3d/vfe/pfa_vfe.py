...
class PCTrans_start(nn.Module):
    ...
    def forward(self, features):
        # pctrans, input (b,12,32)
        #------------------------------------------------------------
        features1 = self.cct1(features,choice="yes")  #  (b,64,32)
...
class pfaVFE(VFETemplate):
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        features1 = self.pctrans_start(features)#(b,64,1)
        # print(features.shape,"*************************************")
        # features = self.pctrans(features)#(b,64,1)
        features1 = features1.view([features1.size()[0], features1.size()[1]])#(b,64)

        for pfn in self.pfn_layers:
            features2 = pfn(features)
        features2 = features2.squeeze()

        features3 = torch.cat([features1,features2],dim = 1)
        # print(features3.shape)

        batch_dict['pillar_features'] = features3#(b,64)
        return batch_dict
