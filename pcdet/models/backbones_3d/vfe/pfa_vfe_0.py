...
class PCTrans(nn.Module):
    def __init__(self):
        ...
        self.pct = PCT(64)
        self.cct1 = CCT(inc=12, channels=64,step=4)
        self.cct1_2 = CCT(inc=12, channels=64,step=4)
...
class PCTrans_start(nn.Module):
    def __init__(self):
        ...
        # self.pct_start = PCT_start(12)
        #------------------------------------------------------------
        self.cct1 = CCT(inc=13, channels=64,step=4)
        self.linear = nn.Linear(13, 64, bias=False)
        self.norm = nn.BatchNorm1d(64, eps=1e-3, momentum=0.01)
        self.act = nn.ReLU()
        self.conv1 = nn.Conv2d(64,64,(2,1))
        self.norm1 = nn.BatchNorm2d(64, eps=1e-3, momentum=0.01)
        #------------------------------------------------------------
        # self.cct1 = CCT(inc=12, channels=12,step=4)
        # self.cct1_2 = CCT(inc=12, channels=12,step=4)
        # self.cct2 = CCT(inc=32, channels=32,step=4)

    def forward(self, features):
        # pctrans, input (b,12,32)
        #------------------------------------------------------------
        features1 = self.cct1(features,choice="nomax")  #  (b,64,32)
        features2 = self.linear(features)  #(b,32,64)
        features2 = self.norm(features2.permute(0, 2, 1)).permute(0, 2, 1) 
        features2 = self.act(features2)
        x_max = torch.max(features2, dim=1, keepdim=True)[0] #(b,1,64)
        x_max = x_max.permute(0,2,1) #(b,64,1)
        x_max = x_max.repeat(1,1,32) #(b,64,32)
        
        
        x_max = torch.unsqueeze(x_max,dim=2)#(b,64,1,32)
        features1 = torch.unsqueeze(features1,dim=2)#(b,64,1,32)


        y = torch.cat((x_max,features1),2)#(b,64,2,32)
        y = self.conv1(y) #(b,64,1,31)
        y = self.norm1(y)
        y = self.act(y)
        y = torch.max(y, dim=3, keepdim=True)[0] #(b,64,1,1)
...
class pfaVFE(VFETemplate):
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        features = self.pctrans_start(features)#(b,64,1)
        # print(features.shape,"*************************************")
