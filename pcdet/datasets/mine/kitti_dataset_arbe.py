...
from ...utils import box_utils, calibration_mine, common_utils, object3d_mine
from ..dataset import DatasetTemplate


class MineDataset_ARBE(DatasetTemplate):
    ...
    def get_lidar(self, idx):
        ...
        #修改1  点数变为5
        points_arbe = np.fromfile(str(lidar_file), dtype=np.float32).reshape(-1, 5)
    ...
    def get_label(self, idx):
        ...
        return object3d_mine.get_objects_from_label(label_file)

    def get_calib(self, idx):
        calib_file = self.root_split_path / 'calib' / ('%s.txt' % idx)
        assert calib_file.exists()
        return calibration_mine.Calibration(calib_file)
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        #做数据增强是读取这里面的文件
        db_info_save_path = Path(self.root_path) / ('mine_dbinfos_%s.pkl' % split)
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=16):
    dataset = MineDataset_ARBE(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    train_split, val_split = 'train', 'val'

    train_filename = save_path / ('mine_infos_%s.pkl' % train_split)
    val_filename = save_path / ('mine_infos_%s.pkl' % val_split)
    trainval_filename = save_path / 'mine_infos_trainval.pkl'
    test_filename = save_path / 'mine_infos_test.pkl'
...
if __name__ == '__main__':
    import sys
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_mine_infos':
        import yaml
        ...
        create_kitti_infos(
            ...
            # fangchange
            data_path=ROOT_DIR / 'data' / 'mine' / 'radar_arbe',
            save_path=ROOT_DIR / 'data' / 'mine' / 'radar_arbe'
        )
