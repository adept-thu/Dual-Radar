...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = ['car', 'pedestrian', 'bicycle', 'motorcycle', 'tricycle', 'bus']
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
    ...
    # for i in range(num_gt):
        if gt_anno["name"][i] == "DontCare":
            dc_bboxes.append(gt_anno["gt_boxes"][i])
...
def compute_statistics_jit(overlaps,
    ...
    # dt_alphas = dt_datas[:, -2]
    # gt_alphas = gt_datas[:, -2]
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        ...
        gt_datas = np.concatenate(
            [gt_annos[i]["location"], gt_annos[i]["dimensions"], gt_annos[i]["rotation_y"][..., np.newaxis]], 1)
