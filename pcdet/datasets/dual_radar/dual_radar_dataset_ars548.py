...
#fangchange
class DualradarDataset_ARS548(DatasetTemplate):
    ...
    def include_kitti_data(self, mode):
        ...
        if self.logger is not None:
            self.logger.info('Total samples for Dual radar dataset: %d' % (len(kitti_infos)))
    ...
    def get_ars(self, idx):
        ars_file = self.root_split_path / 'ars548' / ('%s.bin' % idx)
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            info = {}
            #修改5  将特征数4 改成6
            pc_info = {'num_features': 5, 'ars_idx': sample_idx}
            ...
            if has_label:
                ...
                annotations['gt_boxes_ars'] = gt_boxes_lidar

                info['annos'] = annotations

                if count_inside_pts:
                    points = self.get_ars(sample_idx)
                    calib = self.get_calib(sample_idx)
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        for k in range(len(infos)):
            ...
            info = infos[k]
            sample_idx = info['point_cloud']['ars_idx']
            points = self.get_ars(sample_idx)
            annos = info['annos']
            names = annos['name']
            difficulty = annos['difficulty']
            bbox = annos['bbox']
            gt_boxes = annos['gt_boxes_ars']
            ...
            for i in range(num_obj):
                ...
                if (used_classes is None) or names[i] in used_classes:
                    ...
                    db_info = {'name': names[i], 'path': db_path, 'image_idx': sample_idx, 'gt_idx': i,
                               'box3d_ars': gt_boxes[i], 'num_points_in_gt': gt_points.shape[0],
    ...
    def generate_prediction_dicts(batch_dict, pred_dicts, class_names, output_path=None):
        ...
        def get_template_prediction(num_samples):
            ret_dict = {
                ...
                'score': np.zeros(num_samples), 'boxes_ars': np.zeros([num_samples, 7])
            }
            return ret_dict
        ...
        def generate_single_sample_dict(batch_index, box_dict):
            ...
            pred_dict['boxes_ars'] = pred_boxes
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=16):
    #fangchange
    dataset = DualradarDataset_ARS548(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_dual_radar_infos':
        ...
        create_kitti_infos(
            ...
            # fangchange
            data_path=ROOT_DIR / 'data' / 'dual_radar' / 'radar_ars548',
            save_path=ROOT_DIR / 'data' / 'dual_radar' / 'radar_ars548'
        )
